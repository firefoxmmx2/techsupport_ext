 ...
 */
	 ...
	 *
	 */
	techsupport.systemmanage.SystemWindow = Ext.extend(Ext.Window,{
...
//			this.pager = config.pager;
			...
			techsupport.systemmanage.SystemWindow.superclass.constructor.apply(this,arguments);
		},
...
//				面板里面放置系统所需的属性
				items:[
					      ...
					      {name:'systemcode',fieldLabel:'系统代码',allowBlank:false,blankText:'系统代码不能为空',
					    	  ...
					    	  validator:function(val){
					    		  ...
					    		  $.ajax({url:context_path+'/sysadminDefault/checkSystemcode_systemmanage.action',
					    			  data:{'system.systemcode':val},
					    			  async:false,
					      ...
					      {name:"systemdefine",fieldLabel:"系统定义",allowBlank:true},
					      {name:'parent.systemname',fieldLabel:'上级系统',allowBlank:true,blankText:'上级系统不能为空',readOnly:true},
					      {name:'fullcode',fieldLabel:'系统全码',allowBlank:false,blankText:'系统全码不能为空',readOnly:true},
					      {name:'parent.systemcode',fieldLabel:'上级系统代码',allowBlank:true,blankText:'上级系统代码不能为空',readOnly:true,hidden:true},
					      {name:'parent.isleaf',fieldLabel:'上级系统叶子',hidden:true,readOnly:true},
					      {name:'parent.fullcode',fieldLabel:'上级系统全码',allowBlank:true,blankText:'上级系统全码不能为空',readOnly:true,hidden:true},
					      {name:'nodeorder',fieldLabel:'序列',allowBlank:true,readOnly:true,regex:/^\d*$/,regexText:'序列必须为数字',hidden:true},
					      {name:"isleaf",fieldLabel:"子节点",regex:/^[YN]*$/,hidden:true}
				]
			});
			//添加FORM事件
			this.form_panel.getForm().findField('systemcode').on('blur',function(field){
	    				  	var form = this.form_panel.getForm();
			    		    var parentfullcodeField = form.findField('parent.fullcode');
							var fullcodeField = form.findField('fullcode');
			...
			//初始化表单
			if(this.initRecord){
				this.form_panel.getForm().setValues(buildSubmitParam({},this.initRecord.data));
			}
			...
			techsupport.systemmanage.SystemWindow.superclass.initComponent.apply(this,arguments);
		}
	});


	techsupport.systemmanage.SystemMain = Ext.extend(Ext.Panel, {
		title_base : "系统",
		layout:'border',
		id:'manage_panal',
		defaults:{
		...
		initComponent : function(ct,position) {
			...
			this.action = {
					remove:function(records){
						...
						Ext.Ajax.simpleSubmit({url:self.removeURL,para:params,actionPrefix:self.removePrefix,
...
//								初始化添加机构时候的必须数据
								if(self.detail_store.getCount()){
									...
									self.window.form_panel.getForm().reset();
								}
								
							}
							else{
								Ext.MessageBox.alert("提示","请选择树形菜单中的"+self.title_base);
							}
						}},
						'-',
...
//							执行删除动作
							...
							if(records && records.length > 0){
								...
								Ext.MessageBox.confirm("提示","是否确认要删除选择的记录",function(btn,text){
									if(btn == 'yes'){
...
//				自动适应浏览器宽度
				items:[
			       ...
			       ,
			       {
			    	   ...
			    	   defaults:detail_panel_items_defaults,
			    	   items:[
			    	          {name:'systemdefine',fieldLabel:this.title_base+'定义'}
			    	   ]
			       }
			       ,
			       {
			    	   ...
			    	   defaults:detail_panel_items_defaults,
			    	   items:[
			    	          {name:'nodeorder',fieldLabel:this.title_base+'序列',vtype:"number",vtypeText:"输入必须为数字"}
			    	   ]
			       }
			       ,
			       {
			    	   ...
			    	   defaults:detail_panel_items_defaults,
			    	   items:[
			    	          {name:'picturepath',fieldLabel:this.title_base+'图标路径'}
			    	   ]
			       }
			       ,
			       {
			    	   ...
			    	   defaults:detail_panel_items_defaults,
			    	   items:[
			    	          {name:'isleaf',fieldLabel:this.title_base+'子节点'}
			    	   ]
			       }
			   ]
			});
...
//			----------------------------右边面板-------------------------------------------
		...
		afterRender:function(ct,position){
			...
			techsupport.systemmanage.SystemMain.superclass.afterRender.apply(this,arguments );
